...
export const HANA_BUILT_IN_THEMES = {
    'warm-paper': {
        bg: '#F8F5ED',
        bgCard: '#FCFAF5',
        accent: '#537D96',
...
export function HanaThemeProvider({ mode = 'inherit', theme, className, style, children, 'data-testid': dataTestId = 'hana-plugin-theme', ...rootProps }) {
    const themeId = typeof theme === 'string' ? theme : undefined;
    const tokenStyle = themeStyleFor(mode, theme);
    return (_jsx("div", { ...rootProps, "data-testid": dataTestId, className: cx('hana-plugin-theme', className), "data-hana-theme-mode": mode, "data-hana-theme": mode === 'hana' ? themeId : undefined, style: { ...tokenStyle, ...style }, children: children }));
}
export function themeStyleFor(mode, theme) {
    if (mode === 'inherit')
        return {};
