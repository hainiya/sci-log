...
- **实验记录「AI 主导录入」确认方式**：当前采用**会话内文本确认**（AI 总结成草稿后，回复「记录」确认落库 / 回复「不」取消）。这是 Hana 当前公开 SDK 契约下的可用形态；**最终目标是「交互式卡片」**（可点击的「记录 / 取消」按钮卡片）。由于 Hana 的 `chat.surface` 当前仅支持原生 transcript、富交互卡片与原生组件尚未进入公开 SDK 契约（见宿主 `PLUGINS.md` / `PLUGIN_SDK.md`），此目标将作为后续演进方向，待宿主支持原生交互卡片后升级。
