...
> **当前形态：v2 App（manifestVersion 2，v2.0.0）**。本仓库已从 v1 插件迁移为 v2 App：
> 零构建直接交付（`index.js` 入口 + `lib/` 数据层 + `tools/` 工具 + `ui/panel.html` 静态面板），
> 安装于 `<HANA_HOME>/apps/sci-log`。迁移设计、能力裁剪（自动后台同步 / app 侧 LLM 巡检移除）、
> 数据迁移方式详见 **[README-v2.md](./README-v2.md)**。
>
> 下方 v1 文档与 `src-server/`、`ui/*.tsx` 为 v1 时代遗产，仅作参考（git 历史完整保留 v1 演进）。

---

面向材料专业科研的 **AI 科研辅助工具**：以**实验记录为数据主线**，提供甘特图/日历可视化、指标趋势、Zotero 文献收纳，并让 AI 直接读写这些数据。

- 插件 ID：`sci-log`
- 类型：v2 App（manifestVersion 2，对话工具 + 静态面板）
- 形态：`manifestVersion: 2` · 版本：`2.0.0`
- 宿主：Hana（openhanako）
