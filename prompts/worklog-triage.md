...
```json
{
  ...
  "needRedo": false,
  "redoReason": "若 needRedo 为 true，简述为何需要重做（≤30字）；否则 null",
  "durationHours": 244,
...
- needRedo：仅当该记录清晰地表明所关联的**实验/任务失败或未达预期结果**，且记录关联到某个任务（提供了 taskId 或内容明确指向某任务）时，才置 true；否则一律 false。判断依据须来自记录原文的负面表述（如"失败""未成功""未达标""开裂""无信号""与预期不符""需重做"），不得凭空推断。
- redoReason：needRedo 为 true 时填写简短原因（失败现象/未达预期的指标），≤30 字；否则 null。
